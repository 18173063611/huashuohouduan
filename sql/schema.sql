...
create table if not exists task (
    ...
    provider varchar(50) comment 'AI 服务提供方，例如 VOLCENGINE / VIDU',
    usage_unit varchar(30) comment '用量单位：TOKEN/CHAR/IMAGE/SECOND/PROVIDER_CREDIT/TASK 等',
    estimated_usage decimal(18,4) comment '创建任务时预估用量',
    actual_usage decimal(18,4) comment '任务完成后实际用量',
    estimated_credit_cost bigint not null default 0 comment '创建任务时预估积分成本',
    actual_credit_cost bigint not null default 0 comment '任务完成后实际积分成本',
    settlement_status varchar(30) not null default 'NONE' comment '结算状态：NONE/PRECHARGED/SETTLED/REFUNDED/DEBT 等',
    ...
    key idx_task_provider (provider),
